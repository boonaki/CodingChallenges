...
//P: given an unsorted array of numbers from 0-100, with 1 number missing
//R:return ONLY the number missing
//E:

function missingNo(nums){
    //create array 0-100
    //filter completed array to test if each number ISNT in nums array (if not then add to filtered array)
    //return first value of array
    let completeArray = [...Array(101).keys()]
    return completeArray.filter((num) => !nums.includes(num))[0]
}
